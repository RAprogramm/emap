...
The table is generated by `./rebuild_benchmark.sh`, which copies
[`tests/benchmark.rs`][benchmark], executes it for a few selected capacities
and injects the resulting Markdown between the `<!-- benchmark -->` markers
below.
...
The Criterion targets live under [`benches/`](benches) and require the
`benchmarks` feature. Add the `bench_unchecked` feature flag as well if you
want to include `unsafe` microbenchmarks:
